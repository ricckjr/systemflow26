...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'chk_crm_oportunidades_temperatura_1_100'
      AND conrelid = 'public.crm_oportunidades'::regclass
  ) THEN
    ALTER TABLE public.crm_oportunidades
      ADD CONSTRAINT chk_crm_oportunidades_temperatura_1_100 CHECK (
        temperatura IS NULL OR (temperatura >= 1 AND temperatura <= 100)
      );
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'chk_crm_oportunidades_prev_entrega_month'
      AND conrelid = 'public.crm_oportunidades'::regclass
  ) THEN
    ALTER TABLE public.crm_oportunidades
      ADD CONSTRAINT chk_crm_oportunidades_prev_entrega_month CHECK (
        prev_entrega IS NULL OR date_trunc('month', prev_entrega) = prev_entrega
      );
  END IF;
END $$;
