...
def run(cmd, check=True, cwd=PROJECT_DIR, capture_output=False):
    print(f"\n▶ {cmd_to_str(cmd)}")
...
def run_output(cmd, cwd=PROJECT_DIR):
    result = subprocess.run(
        cmd,
        cwd=cwd,
        capture_output=True,
...
def check_dirty_files():
    """Verifica se há arquivos modificados ou não rastreados no git."""
    result = subprocess.run(
...
def wait_healthy():
    ...
    print(
        ...
        f"({HEALTH_RETRIES}x a cada {HEALTH_INTERVAL}s)..."
    )

    for attempt in range(1, HEALTH_RETRIES + 1):
        result = run(
            ["curl", "-k", "-sf", HEALTH_URL, "-o", "/dev/null"],
            check=False
        )
...
def show_logs():
    ...
    run(COMPOSE_CMD + ["logs", "backend", "--tail", "100"], check=False)

    print("\n📄 Últimos logs do frontend:")
    run(COMPOSE_CMD + ["logs", "frontend", "--tail", "50"], check=False)
...
build_result = run(COMPOSE_CMD + ["build"], check=False)

if build_result.returncode != 0:
    print(f"\n❌ Build falhou. Rollback para {old_hash}...")
    run(["git", "reset", "--hard", old_hash])
    sys.exit(1)
