...
function isAbortLikeError(err: unknown) {
  ...
  if (message.includes('ERR_ABORTED') || message.includes('net::ERR_ABORTED')) return true
  if (message.toLowerCase().includes('the user aborted a request')) return true
  const causeMessage = String(anyErr?.cause?.message || '')
  if (causeMessage.includes('AbortError')) return true
  if (causeMessage.includes('ERR_ABORTED') || causeMessage.includes('net::ERR_ABORTED')) return true
  return false
}
